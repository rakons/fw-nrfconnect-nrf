...
/**
 * @brief Register event listener for the remote event.
 *
 ...
 * @return See @ref event_manager_proxy_register_listener.
 */
#define EVENT_MANAGER_PROXY_REGISTER_LISTENER(instance, ename) \
	event_manager_proxy_register_listener((instance), _EVENT_ID(ename), STRINGIFY(ename))
 ...
 *
 * @retval 0 On success.
 * @retval other errno codes depending on the implementation in IPC service.
 */
int event_manager_proxy_add_remote(const struct device *instance);

/**
 * @brief Register as an event listener
 *
 ...
 * @ref event_manager_proxy_add_remote and @ref event_manager_proxy_register_listener
